...
| 2026-07-27 | [keeplin#186](https://github.com/jsunyermias/keeplin/pull/186) · [keeplin-srv#95](https://github.com/jsunyermias/keeplin-srv/pull/95) — `orden-04f`, the checks on this file and the CI gate | Claude | No independent review, and the family the checks constrain wrote them. Three things need an outsider. The gate parses a free-text pull request body by string prefix, so it breaks quietly if anyone rewords the template labels. The filler-value list decides what counts as an unanswered field, and a short legitimate answer could be rejected by it. And `ci.yml` is not a shared file: the `review` job was inserted into each repository separately, so nothing notices if the two drift apart. | maintainer sweep, pending |
