...
| [0003 — Per-user quota enforcement under concurrency](0003-per-user-quota-serialization.md) | proposed | per-user advisory lock so the two quota check-then-act paths hold under concurrent requests, which ADR 0002's re-verification rule cannot close | [keeplin-srv#142](https://github.com/jsunyermias/keeplin-srv/issues/142) | none; proposed does not authorize implementation |
