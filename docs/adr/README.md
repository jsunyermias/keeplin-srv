...
| [0008 — Sync-path quota and journal cost](0008-sync-path-quota-and-journal-cost.md) | proposed; supersedes `0003` in part (only its property that each transaction acquires at most one advisory lock) | enforces `max_user_storage_bytes` in resource projection, dead-letters permanent quota refusals, orders the two required advisory locks, and removes JSON-array amplification from the server-local journal encoding without changing the wire | [keeplin-srv#145](https://github.com/jsunyermias/keeplin-srv/issues/145) | none — proposed |
...
`0008` is proposed and supersedes `0003` in part: only its property that each transaction acquires
at most one advisory lock. `0003`'s quota serialization decision otherwise stands in full:
