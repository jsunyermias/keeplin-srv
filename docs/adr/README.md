...
| [0003 — Making per-user quotas hold](0003-per-user-quota-serialization.md) | **accepted**; superseded by `0008` in part (only its property that each transaction acquires at most one advisory lock) | enforcement at every path that creates a counted object, plus a per-user advisory lock so the check and the write it authorizes are mutually exclusive | [keeplin-srv#142](https://github.com/jsunyermias/keeplin-srv/issues/142), [keeplin-srv#145](https://github.com/jsunyermias/keeplin-srv/issues/145) | [keeplin-srv#143](https://github.com/jsunyermias/keeplin-srv/pull/143) |
...
| [0007 — A login must re-verify its credential inside the operation that mints the session](0007-login-credential-reverification.md) | **accepted**; supersedes `0005` in part (its handler enumeration becomes ten with `login`) | adds `login` to the serializable participant set for the credential-atomicity reason `0002` states, so a completed password reset invalidates a session being minted concurrently; the `delete_account` re-verification shape, with the residual it does not close recorded rather than implied | [keeplin-srv#151](https://github.com/jsunyermias/keeplin-srv/issues/151) | [keeplin-srv#157](https://github.com/jsunyermias/keeplin-srv/pull/157) |
| [0008 — Sync-path quota and journal cost](0008-sync-path-quota-and-journal-cost.md) | **accepted**; supersedes `0003` in part (only its property that each transaction acquires at most one advisory lock) | enforces `max_user_storage_bytes` in resource projection, dead-letters permanent quota refusals, orders the two required advisory locks, and removes JSON-array amplification from the server-local journal encoding without changing the wire | [keeplin-srv#145](https://github.com/jsunyermias/keeplin-srv/issues/145) | [keeplin-srv#158](https://github.com/jsunyermias/keeplin-srv/pull/158) |
...
`0008` is accepted and supersedes `0003` in part: only its property that each transaction acquires
at most one advisory lock. Its decision authorizes the synchronization-path work but does not
perform it: no code in its acceptance pull request implements the projection-side quota, the lock
ordering or the journal re-encoding, which remain keeplin-srv#145's synchronization half. `0003`'s quota serialization decision otherwise stands in full:
enforcement at every path that creates a counted object; the lock before the deciding read; that
read and its authorized write on the same transaction; per-user, per-quota keying and domain
separation; byte-identical HTTP `507`; and no serializable retry or `503` on quota paths. `0008` has
no canonical or companion ADR in `keeplin`: projection, quota, journal physical encoding and job
classification are server-local, while the serialized `Change` remains byte-identical on the wire
and no shared wire, format or `keeplin-core` surface moves.
