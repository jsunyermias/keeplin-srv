...
| [0006 — Making a journaled change reach its projection](0006-durable-projection.md) | proposed | a durable per-batch projection marker written in the journal transaction and re-driven by a bounded sweep, plus a reconciliation command, so a journaled change is projected or visibly marked failed; answers the exhaustion sentence `0005` defers, and forecloses no option of `0004` | [keeplin-srv#75](https://github.com/jsunyermias/keeplin-srv/issues/75) | none; proposed does not authorize implementation |
...
never re-sends, so a rolled-back journal append loses a batch irrecoverably. It also rejects the full
projection queue an earlier draft proposed, because the issue's bar is a queue only if a queue is
demonstrably needed and the smaller durable design meets every force. It inherits `0004`'s verified facts and answers the exhaustion sentence `0005` defers.
